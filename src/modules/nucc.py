...
CABOCHA_DICTIONARY_PATH  = '/usr/local/lib/mecab/dic/mecab-ipadic-neologd'
CORPUS_PATH = str(Path(os.path.dirname(os.path.abspath("__file__"))).parent.parent.joinpath('data', 'modnucc'))

with open(Path(CORPUS_PATH).joinpath('frequency.json'), encoding='utf-8') as f:
    frequency_dic = json.loads(f.read())

cabocha_parser = CaboCha.Parser(f'-d {CABOCHA_DICTIONARY_PATH}')
...
class Word:
    ...
    def __init__(self, lemma: str, wordcategory: str, subwordcategory:str , isbunsetsuhead:bool = False):
        ...
        # 否定語を同じ文節に含むか
        self.isreverse: bool = self.isreverseword()
        # 接続詞であるか
        self.isconnector: bool = (self.wordcategory == '接続詞')

    def isreverseword(self):
